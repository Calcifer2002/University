...
function CohortPage() {
  ...
  return (
    <div className="container">
      ...
      <h1 className="mt-3 mb-4">{cohort.id} - Year {cohort.year}</h1>
      <h2 className="mt-3 mb-4">{cohort.name}</h2>
      <h2 className="mt-3 mb-4">Degree - <Link to={`/degrees/${cohort.degree.split("/")[ cohort.degree.split("/").length - 2]}`}>
      {cohort.degree.split("/")[ cohort.degree.split("/").length - 2]}
                </Link> </h2>
                <Link to={`/cohort/modules/${id}`}>
          View relevant modules
        </Link>
      <h2>Students</h2>
      <Link to={`/student/add`}>
          Add
